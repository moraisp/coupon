...
(function patchConsole() {
    ...
    console.log = (...a) => { _log(...a); fwd("log", a); };
...
function getAccessibleDocs() {
    const docs = [{ doc: document, frameIdx: 0 }];
    const frames = Array.from(document.querySelectorAll("iframe"));
    frames.forEach((fr, i) => {
        try {
            const d = fr.contentDocument;
            if (d)
                docs.push({ doc: d, frameIdx: i + 1 });
        }
...
function encodeSelector(frameIdx, selector) {
    return `${frameIdx}:${selector}`;
}
/** Resolve an encoded selector back to an element */
function resolveSelector(encoded) {
    ...
    if (!entry)
        return null;
    const el = entry.doc.querySelector(selector);
    if (!el)
        return null;
...
function getPageInfo() {
    ...
    for (const { doc, frameIdx } of getAccessibleDocs()) {
        // Inputs
        const inputEls = Array.from(doc.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="reset"]), textarea'));
        inputEls.forEach((el) => {
        ...
        // Buttons
        const buttonEls = Array.from(doc.querySelectorAll('button, input[type="submit"], input[type="button"], input[type="reset"]'));
        ...
        const linkEls = Array.from(doc.querySelectorAll("a, button, [role='button'], span, div"))
            .filter((el) => {
...
// ── Tool: fill_input ──────────────────────────────────────────────────────
function fillInput(encodedSelector, value) {
    const resolved = resolveSelector(encodedSelector);
    if (!resolved)
        throw new Error(`fill_input: no element found for "${encodedSelector}"`);
    const { el } = resolved;
    const input = el;
    input.focus();
...
// ── Tool: click_element ───────────────────────────────────────────────────
async function clickElement(encodedSelector) {
    const resolved = resolveSelector(encodedSelector);
    if (!resolved)
        throw new Error(`click_element: no element found for "${encodedSelector}"`);
    const { el } = resolved;
...
// ── Tool: query_dom ───────────────────────────────────────────────────────
function queryDom(encodedSelector) {
    const allResults = [];
    ...
    for (const { doc, frameIdx } of getAccessibleDocs()) {
        if (targetFrame !== null && frameIdx !== targetFrame)
            continue;
        try {
...
function getPageText() {
    ...
    for (const { doc, frameIdx } of getAccessibleDocs()) {
        ...
        const toastText = Array.from(doc.querySelectorAll(toastSelectors))
        ...
        if (merged)
            texts.push(frameIdx === 0 ? merged : `[frame ${frameIdx}] ${merged}`);
    }
    return texts.join("\n---\n");
}
// ── Tool: scrape_pelando_coupons ────────────────────────────────────────
function scrapePelandoCoupons() {
    const codes = new Set();
    const maskedEls = Array.from(document.querySelectorAll("[data-masked]"));
...
function scrapePelandoStoreLinks() {
    ...
    anchors.forEach((a) => {
        ...
        try {
            ...
            if (!abs.pathname.startsWith("/cupons-de-descontos/"))
                return;
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    switch (message.type) {
        case "QUERY_DOM":
            sendResponse(queryDom(message.selector));
            return false;
        ...
        case "FILL_INPUT":
            try {
                fillInput(message.selector, message.value);
