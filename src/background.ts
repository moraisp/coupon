...
function postDone(result: CouponResult, code: string, code2: string | undefined, planUsed: boolean, inconsistent: boolean, run1Result?: CouponResult): void {
  ...
  activePort?.postMessage({ type: "DONE", result, code, code2, planUsed, inconsistent, run1Result } as PortMessage);
}
...
async function runSinglePass(
  ...
  systemPrompt: string,
  runLabel: string,
): Promise<{ result: CouponResult; messages: OllamaMsg[] }> {
